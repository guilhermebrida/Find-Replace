...
class VariosArquivos():
    def __init__(self):
        customtkinter.CTkLabel(root, text= "Find: ").pack()
        self.find = Entry(root)
        self.find.bind('<Return>')
        self.find.pack()

        customtkinter.CTkLabel(root, text= "Replace: ").pack()
        self.replace = Entry(root)
        self.replace.bind('<Return>')
        self.replace.pack()

        customtkinter.CTkButton(root, text='Do it!',command=lambda: self.Replace()).pack(pady=10, ipadx=10)
